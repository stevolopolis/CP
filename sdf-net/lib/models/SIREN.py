...
class SIREN(BaseLOD):
    ...
    def __init__(self, args):
        super().__init__(args)
        self.siren_configs = self.args
        num_layers = self.siren_configs.num_layers
        dim_hidden = self.siren_configs.dim_hidden
        w0 = self.siren_configs.w0
        w0_initial = self.siren_configs.w0_initial
        use_bias = self.siren_configs.use_bias
        ...
        for ind in range(num_layers - 1):
            ...
            layer_dim_in = self.dim_in if is_first else dim_hidden
        ...
        self.last_layer = SirenLayer(
            dim_in=dim_hidden, dim_out=self.dim_out, w0=w0, use_bias=use_bias, is_last=True
        )
